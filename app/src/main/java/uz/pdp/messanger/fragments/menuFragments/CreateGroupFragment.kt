...
import uz.pdp.messanger.Room.Database.AppDatabase_Impl
import uz.pdp.messanger.adapters.MultiSeclectionAdapter
import uz.pdp.messanger.databinding.FragmentCreateGroupBinding
...
class CreateGroupFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.save.setOnClickListener {
            if(uri!=null && binding.name.text.isNotEmpty() && selections.size>0){
                ...
                Toast.makeText(requireContext(), "Select group image,enter group name and add member", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun creategroup() {
        ...
        progressDialog.setMessage("Group is creating")
        progressDialog.show()
        var ref=FirebaseDatabase.getInstance().getReference("Groups")
        var key=ref.push().key
        var stroage=FirebaseStorage.getInstance().getReference("GroupProfileImages/${firebaseAuth.currentUser?.uid}/${System.currentTimeMillis()}")
        stroage.putFile(uri!!).addOnSuccessListener(object:OnSuccessListener<UploadTask.TaskSnapshot>{
              ...
              var task=  p0?.metadata?.reference?.downloadUrl
                task?.addOnSuccessListener(object :OnSuccessListener<Uri>{
                    override fun onSuccess(p0: Uri?) {
                        ...
                        var group=Groups(binding.name.text.toString(),photoUrl,key,selections)
                        ref.child(key!!).setValue(group).addOnCompleteListener{
                            if(it.isSuccessful){
