...
import uz.pdp.messanger.databinding.GroupRecieverItemBinding
import uz.pdp.messanger.databinding.GroupSenderItemBinding
...
class GroupMessageAdapter(var list: ArrayList<Groupmessage>, var currentuserid:String, var groups: Groups):RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    private var simpleDateFormat = SimpleDateFormat("dd.MM.yyyy HH:mm")
    var firebaseDatabase=FirebaseDatabase.getInstance()
    lateinit var ref:DatabaseReference
    var firebaseAuth=FirebaseAuth.getInstance()
    inner class Sendtext(var itemRvBinding: GroupSenderItemBinding):
             RecyclerView.ViewHolder(itemRvBinding.root){
         fun onBind(message: Groupmessage,position: Int) {

             if(position==0){
                 itemRvBinding.day.visibility=View.VISIBLE
                 itemRvBinding.day.text=simpleDateFormat.format(message.date).substring(0,10)
             } else if(position>0 && (simpleDateFormat.format(list[position].date).substring(0,10)!=
                         simpleDateFormat.format(list[position-1].date).substring(0,10))){
                 itemRvBinding.day.visibility=View.VISIBLE
                 itemRvBinding.day.text=simpleDateFormat.format(message.date).substring(0,10)

             } else {
                 itemRvBinding.day.visibility=View.GONE
             }
...
//
     ...
     inner class Recievetext(var itemRvBinding: GroupRecieverItemBinding):
             RecyclerView.ViewHolder(itemRvBinding.root){
         fun onBind(message: Groupmessage,position: Int) {

             ref=firebaseDatabase.getReference("Groups/${groups.key}/groupmessages/${message.key}/check")
             ref.setValue("true")
             ...
             if(message.type=="text"){
                 ...
                 itemRvBinding.text.visibility=View.VISIBLE
                 itemRvBinding.messagetext.text = message.message
                 itemRvBinding.usname.text=message.sendUser?.firstname
                 val format = simpleDateFormat.format(message.date)
                 itemRvBinding.date.text = format.substring(11)
                 Glide.with(itemRvBinding.usimage.context).load(message.sendUser?.photoUrl).apply(RequestOptions().placeholder(R.drawable.ic_man__1_))
                     .into(itemRvBinding.usimage)

             } else if(message.type=="audio"){
                 itemRvBinding.image.visibility=View.GONE
                 itemRvBinding.audio.visibility=View.VISIBLE
                 itemRvBinding.text.visibility=View.GONE
                 itemRvBinding.usnameaudio.text=message.sendUser?.firstname
                itemRvBinding.voicePlayerView.setAudio(message.message)
                 val format = simpleDateFormat.format(message.date)
                 itemRvBinding.dateaudio.text = format.substring(11)
                 Glide.with(itemRvBinding.usimage.context).load(message.sendUser?.photoUrl).apply(RequestOptions().placeholder(R.drawable.ic_man__1_))
                     .into(itemRvBinding.usimageaudio)
             }
             else if(message.type=="image"){
...
//                 circularProgressDrawable.start()
                 ...
                 itemRvBinding.audio.visibility=View.GONE
                 itemRvBinding.text.visibility=View.GONE
                 itemRvBinding.usnameimage.text=message.sendUser?.firstname
                 val format = simpleDateFormat.format(message.date)
                 itemRvBinding.dateimage.text = format.substring(11)
                 Glide.with(itemRvBinding.usimage.context).load(message.sendUser?.photoUrl).apply(RequestOptions().placeholder(R.drawable.ic_man__1_))
                     .into(itemRvBinding.usimageimage)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == 0) {
            return Sendtext(
                GroupSenderItemBinding.inflate(
            ...
            return Recievetext(
                GroupRecieverItemBinding.inflate(
                    LayoutInflater.from(parent.context),
                    parent,
                    false
                )
