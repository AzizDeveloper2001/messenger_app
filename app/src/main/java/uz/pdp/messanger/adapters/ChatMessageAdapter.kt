...
import uz.pdp.messanger.databinding.ItemdateBinding
...
class ChatMessageAdapter(var list: ArrayList<Message>,var currentuserid:String,var userData: UserData):RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    inner class Sendtext(var itemRvBinding: SendmeBinding):
         ...
         fun onBind(message: Message,position: Int) {
             ...
             if(message.type=="text"){
                 ...
                 itemRvBinding.voicePlayerView.playSoundEffect(5)
...
//
     ...
     inner class Recievetext(var itemRvBinding: SendFromMeBinding):
         ...
         fun onBind(message: Message,position: Int) {
             ...
             ref=firebaseDatabase.getReference("Message/${userData.uid}/${firebaseAuth.currentUser?.uid}/${message.key}/check")
             ref.setValue("true")
             var r=firebaseDatabase.getReference("Message/${firebaseAuth.currentUser?.uid}/${userData.uid}/${message.key}/check")
             r.setValue("true")
             if(position==0){
                 itemRvBinding.day.visibility=View.VISIBLE
                 itemRvBinding.day.text=simpleDateFormat.format(message.date).substring(0,10)
...
//                 circularProgressDrawable.start()
    ...
    override fun getItemViewType(position: Int): Int {
        return if(list[position].from==currentuserid){
            0
        } else{
            1
        }
