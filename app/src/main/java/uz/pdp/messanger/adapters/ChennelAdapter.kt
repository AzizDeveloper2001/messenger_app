...
class ChennelAdapter(var list:ArrayList<Chennel>, var listener:ChennelAdapter.onItemClick):RecyclerView.Adapter<ChennelAdapter.Vh>() {
    ...
    inner class Vh(var itemRvBinding: ChatcontactsrvItemBinding):
         ...
         fun onBind(group: Chennel, position: Int) {
            ...
            itemRvBinding.onOff.visibility=View.INVISIBLE
             ...
             var r=firebaseDatabase.getReference("Chennels/${group.key}/chennelmessages/")
             var count=0
             r.addListenerForSingleValueEvent(object : ValueEventListener {
                 ...
                 override fun onDataChange(snapshot: DataSnapshot) {
                     ...
                     for (child in snapshot.children) {
                         var mesage=child.getValue(ChennelMessage::class.java)
                         if (mesage != null) {
    ...
    fun updatelist(newList: ArrayList<Chennel>) {
        list= ArrayList()
        list.addAll(newList)
    ...
    interface onItemClick{
        fun OnItemClick(userData: Chennel,position: Int)
